/**
 * Pure Document PDF Generation Projections
 *
 * Transforms entity domain records into structured props for `PdfPreviewModal`.
 * Pure projection layer: cross-table context is resolved via `documentContext.js`.
 *
 * TextileOps Architecture: Deep Module / Pure Document Projections
 */
...
export function safeFormatDate(val, fmt = 'dd/MM/yyyy HH:mm') {
  if (!val) return '—'
  try {
...
export function extractHiddenValue(note = '', prefix = '') {
  if (!note || typeof note !== 'string') return ''
 ...
 */
export function normalizeImagesList(rawImages, defaultCaption = 'รูปถ่ายตัวอย่าง / Inspection Photo') {
  if (!rawImages) return []
...
export function generateMachinePdfProps(mc, context = {}) {
  ...
  const matchedCyl = findMachineCylinderMatch(mc, cylinders)

  // Cross-reference center checks / needle conditions for last inspection date
  const centerChecks = resolveCenterChecks(context)
  const needleConditions = resolveNeedleConditions(context)
...
export function generateCylinderPdfProps(cyl, context = {}) {
  ...
  // Machine matching from context or fallback
  const machines = resolveMachines(context)
  const matchedMc = findCylinderMachineMatch(cyl, machines)

  // Inspection matching from context or fallback
  const centerChecks = resolveCenterChecks(context)
  const needleConditions = resolveNeedleConditions(context)
...
export function generateWorkOrderPdfProps(wo, context = {}) {
  ...
  const repairRequests = resolveRepairRequests(context)
...
export function generateNeedleConditionPdfProps(needle, historyList = [], context = {}) {
  ...
  return {
    ...
    images,
    signatories: [
      { title: 'ผู้ตรวจเช็คสภาพเข็ม', name: needle.inspector || '', date: safeFormatDate(needle.doc_date, 'dd/MM/yyyy') },
      { title: 'ช่างประจำแผนก', name: '', date: '' },
      { title: 'หัวหน้างานเทคนิค', name: '', date: '' },
      { title: 'ผู้จัดการฝ่ายผลิต', name: '', date: '' },
    ],
  }
}
