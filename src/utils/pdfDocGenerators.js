...
export function generateRepairRequestPdfProps(req) {
  ...
  return {
    ...
    docNo: req.RequestNo || req.code || `REQ-${req.id || req._id}`,
    docDate: req.created_at || new Date(),
    status: req.status || 'รอดำเนินการ',
    priority: req.urgency || req.priority || 'ปกติ',
    remarks: req.description || req.symptom || '',
    sections: [
      {
        title: 'ข้อมูลการแจ้งซ่อม (Repair Request Details)',
        fields: [
          { label: 'เลขที่ใบแจ้งซ่อม (Req No.)', value: req.RequestNo || req.code, mono: true },
          { label: 'วันที่แจ้งซ่อม (Date)', value: req.created_at ? format(new Date(req.created_at), 'dd/MM/yyyy HH:mm') : '—' },
          { label: 'ผู้แจ้งซ่อม (Reporter)', value: req.reporter_name || req.CreatedBy || '—' },
          { label: 'เบอร์ติดต่อ (Phone)', value: req.reporter_phone || '—' },
          { label: 'เครื่องจักรที่แจ้งซ่อม (Machine)', value: req.machine_id || req.mc || req.serial, mono: true },
          { label: 'สถานที่ติดตั้ง (Location)', value: req.location || '—' },
          { label: 'ระดับความเร่งด่วน (Urgency)', value: req.urgency || req.priority || 'ปกติ' },
      ...
      },
      {
        ...
        fields: [
          { label: 'หัวข้อปัญหา / อาการเสีย', value: req.title || req.issue || 'เครื่องจักรขัดข้อง', full: true },
          { label: 'รายละเอียดปัญหา (Symptom Detail)', value: req.description || req.symptom || '—', full: true },
        ],
      },
    ],
    signatories: [
      { title: 'ผู้แจ้งซ่อม (Operator)', name: req.reporter_name || '', date: '' },
      { title: 'ช่างผู้รับเรื่องซ่อม', name: req.technician_name || '', date: '' },
      { title: 'หัวหน้างานตรวจสอบ', name: '', date: '' },
      { title: 'ผู้จัดการฝ่ายซ่อมบำรุง', name: '', date: '' },
    ],
  }
}
